/**
 * api-settings-home — GET /api/settings-home/tree?home=dsi|dsh&path=<rel>
 * One directory level of a settings home for the session-less explorer.
 *
...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		const { listSettingsHomeDir } = await import('$lib/server/settings-home.js');
		const listing = await listSettingsHomeDir(home, rel);
		...
		if (code === 'ENOENT') {
			return json({ ok: false, error: { code: 'not-found', message: 'no such directory' } }, { status: 404 });
		}
