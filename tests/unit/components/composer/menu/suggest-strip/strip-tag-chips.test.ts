...
import StripTagChips from '$lib/components/composer/menu/suggest-strip/StripTagChips.svelte';
...
describe('5.1-T — StripTagChips (D10)', () => {
	...
	// DIRECT mount: the toggle handler fires with the chip's OWN word
	it('DIRECT mount: toggling a chip reports THAT word up through ontoggle', async () => {
		const toggled: string[] = [];
		const target = document.createElement('div');
		document.body.appendChild(target);
		const instance = mount(StripTagChips, {
			target,
			props: { recTags: ['git', 'rca'], checked: ['git'], ontoggle: (w: string) => toggled.push(w) }
		});
		flushSync();
		const labels = Array.from(target.querySelectorAll('.strip-tag-chip'));
		// the pre-checked chip carries the on class (the true arm) ...
		expect(labels[0]!.classList.contains('on')).toBe(true);
		// ... and clicking the unchecked chip reports ITS word
		labels[1]!.querySelector('input')!.click();
		flushSync();
		expect(toggled).toEqual(['rca']);
		unmount(instance);
		target.remove();
	});

	// The compiled chip text is `word ?? ''` — a nullish recTags entry (a
	// wire bug, but the component defends) renders an EMPTY chip, no crash.
	it('DIRECT mount: a nullish recTags entry degrades to an empty chip label', async () => {
		const target = document.createElement('div');
		document.body.appendChild(target);
		const instance = mount(StripTagChips, {
			target,
			props: { recTags: [undefined as unknown as string], checked: [], ontoggle: () => {} }
		});
		flushSync();
		const labels = Array.from(target.querySelectorAll('.strip-tag-chip'));
