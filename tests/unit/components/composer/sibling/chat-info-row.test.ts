 ...
 * ContextConsumption display modes, Composer's row composition
 * (token · model · context), ModelSelector menuUp direction.
 */
import { mount, unmount, flushSync } from 'svelte';
import { describe, expect, it, vi } from 'vitest';
import TokenCounter from '$lib/components/composer/sibling/TokenCounter.svelte';
import ContextConsumption from '$lib/components/composer/sibling/ContextConsumption.svelte';
...
describe('Composer info row', () => {
	function input(props: Record<string, unknown> = {}) {
		return mountInto(Composer, {
